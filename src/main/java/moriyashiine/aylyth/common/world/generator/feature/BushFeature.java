	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		for (int i = 0; i < clusters; ++i) {
			...
			for (BlockPos position : positions) {
				if (position.getSquaredDistance(blockPos) <= (double) (f * f) && ModBlocks.AYLYTH_BUSH.canPlaceAt(ModBlocks.AYLYTH_BUSH.getDefaultState(), structureWorldAccess, position)) {
