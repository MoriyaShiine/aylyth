	...
	public boolean generate(FeatureContext<SeepFeatureConfig> context) {
		...
		for (int xOffset = -2; xOffset < 2; xOffset++) {
			for (int zOffset = -2; zOffset < 2; zOffset++) {
				...
				for (origin = origin.add(xOffset, world.getTopY(Heightmap.Type.WORLD_SURFACE_WG, context.getOrigin().getX(), context.getOrigin().getZ()) - 3, zOffset); origin.getY() < topY; origin = origin.up()) {
