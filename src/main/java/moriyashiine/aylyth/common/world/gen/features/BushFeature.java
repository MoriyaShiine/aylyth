	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		for (int i = 0; i < clusters; ++i) {
			...
			for (BlockPos position : positions) {
				if (position.getSquaredDistance(blockPos) <= (double) (f * f) && structureWorldAccess.testBlockState(position, blockState -> blockState.isReplaceable() && AylythBlocks.AYLYTHIAN_BUSH.getDefaultState().canPlaceAt(structureWorldAccess, position))) {
					structureWorldAccess.setBlockState(position, AylythBlocks.AYLYTHIAN_BUSH.getDefaultState(), Block.NOTIFY_LISTENERS);
					if (structureWorldAccess.isAir(position.up())) {
						structureWorldAccess.setBlockState(position.up(), AylythBlocks.AYLYTHIAN_BUSH.getDefaultState().with(AylythianBushBlock.LEAFY, true), Block.NOTIFY_LISTENERS);
					}
				}
			}
