...
public class YmpeInfestationComponent implements AutoSyncedComponent, ServerTickingComponent {
	...
	public void serverTick() {
		...
		if (obj.getWorld().getRegistryKey() == AylythDimensionData.WORLD) {
		...
		if (getInfestationTimer() >= TIME_UNTIL_STAGE_INCREASES) {
			...
			if (getStage() >= 6) {
				obj.damage(obj.getWorld().aylythDamageSources().ympe(), Float.MAX_VALUE);
			}
		}
	...
	public void setStage(byte stage) {
		this.stage = stage;
		AylythEntityComponents.YMPE_INFESTATION.sync(obj);
