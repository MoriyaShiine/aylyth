...
public final class AylythStructurePoolBootstrap {
    ...
    public static void bootstrap(Registerable<StructurePool> context) {
        ...
        context.register(AylythStructurePools.BLACK_WELL, single(emptyPool, Aylyth.id("black_well"), blackWell, StructurePool.Projection.TERRAIN_MATCHING, Optional.empty()));
    }

    private static StructurePool single(
        ...
        return new StructurePool(fallback, List.of(Pair.of(fromId(id, processorList, projection, overrideLiquidSettings), 1)));
    }

    private static SinglePoolElement fromId(
            Identifier id,
            RegistryEntry<StructureProcessorList> processorList,
            StructurePool.Projection projection,
            Optional<StructureLiquidSettings> overrideLiquidSettings
    ) {
        return SinglePoolElementInvoker.invokeInit(Either.left(id), processorList, projection, overrideLiquidSettings);
    }
}
