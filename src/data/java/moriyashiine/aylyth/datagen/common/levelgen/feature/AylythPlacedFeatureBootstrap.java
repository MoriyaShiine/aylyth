...
import static moriyashiine.aylyth.common.data.levelgen.AylythPlacements.*;
...
public final class AylythPlacedFeatureBootstrap {
    ...
    public static void bootstrap(Registerable<PlacedFeature> context) {
        ...
        var aylythianDarkOak = features.getOrThrow(AylythConfiguredFeautres.AYLYTHIAN_DARK_OAK);
        var aylythianMegaDarkOak = features.getOrThrow(AylythConfiguredFeautres.AYLYTHIAN_MEGA_DARK_OAK);
        var ympe = features.getOrThrow(AylythConfiguredFeautres.YMPE_TREE);
        var bigYmpe = features.getOrThrow(AylythConfiguredFeautres.BIG_YMPE_TREE);
        var pomegranate = features.getOrThrow(AylythConfiguredFeautres.POMEGRANATE_TREE);
        var writhewood = features.getOrThrow(AylythConfiguredFeautres.WRITHEWOOD_TREE);
        var spring = features.getOrThrow(AylythConfiguredFeautres.SPRING);
        var bushes = features.getOrThrow(AylythConfiguredFeautres.BUSHES);
        var oakLeafPile = features.getOrThrow(AylythConfiguredFeautres.OAK_LEAF_PILE);
        var ympeLeafPile = features.getOrThrow(AylythConfiguredFeautres.YMPE_LEAF_PILE);
        var oakStrewnLeaves = features.getOrThrow(AylythConfiguredFeautres.OAK_STREWN_LEAVES);
        var ympeStrewnLeaves = features.getOrThrow(AylythConfiguredFeautres.YMPE_STREWN_LEAVES);
        var aylythWeeds = features.getOrThrow(AylythConfiguredFeautres.AYLYTH_WEEDS);
        var marigolds = features.getOrThrow(AylythConfiguredFeautres.MARIGOLDS);
        var shelfJackOlanternMushrooms = features.getOrThrow(AylythConfiguredFeautres.SHELF_JACK_O_LANTERN_MUSHROOMS);
        var antlerShootsWater = features.getOrThrow(AylythConfiguredFeautres.ANTLER_SHOOTS_WATER);
        var antlerShoots = features.getOrThrow(AylythConfiguredFeautres.ANTLER_SHOOTS);
        var smallWoodyGrowthWater = features.getOrThrow(AylythConfiguredFeautres.SMALL_WOODY_GROWTH_WATER);
        var largeWoodyGrowthWater = features.getOrThrow(AylythConfiguredFeautres.LARGE_WOODY_GROWTH_WATER);
        var oakSeep = features.getOrThrow(AylythConfiguredFeautres.OAK_SEEP);
        var spruceSeep = features.getOrThrow(AylythConfiguredFeautres.SPRUCE_SEEP);
        var darkOakSeep = features.getOrThrow(AylythConfiguredFeautres.DARK_OAK_SEEP);
        var ympeSeep = features.getOrThrow(AylythConfiguredFeautres.YMPE_SEEP);
        var deepRoofTrees = features.getOrThrow(AylythConfiguredFeautres.DEEP_ROOF_TREES);
        var coniferousDeepRoofTrees = features.getOrThrow(AylythConfiguredFeautres.CONIFEROUS_DEEP_ROOF_TREES);
        var copseTrees = features.getOrThrow(AylythConfiguredFeautres.COPSE_TREES);
        var deepWoodTrees = features.getOrThrow(AylythConfiguredFeautres.DEEPWOOD_TREES);
        var coniferousCopseTrees = features.getOrThrow(AylythConfiguredFeautres.CONIFEROUS_COPSE_TREES);
        var coniferousDeepWoodTrees = features.getOrThrow(AylythConfiguredFeautres.CONIFEROUS_DEEPWOOD_TREES);
        var overgrowthClearingTrees = features.getOrThrow(AylythConfiguredFeautres.OVERGROWTH_CLEARING_TREES);
        var mireWaterTrees = features.getOrThrow(AylythConfiguredFeautres.MIRE_WATER_TREES);
        var mireLandTrees = features.getOrThrow(AylythConfiguredFeautres.MIRE_LAND_TREES);
        var redMushroomPatches = features.getOrThrow(AylythConfiguredFeautres.RED_MUSHROOM_PATCHES);
        var brownMushroomPatches = features.getOrThrow(AylythConfiguredFeautres.BROWN_MUSHROOM_PATCHES);
        var glowLichen = features.getOrThrow(AylythConfiguredFeautres.GLOW_LICHEN);
        var shelfJackOLanternMushroomPatches = features.getOrThrow(AylythConfiguredFeautres.SHELF_JACK_O_LANTERN_MUSHROOM_PATCHES);
        var ghostcapMushroomPatches = features.getOrThrow(AylythConfiguredFeautres.GHOSTCAP_MUSHROOM_PATCHES);
        var pomegranateTree = features.getOrThrow(AylythConfiguredFeautres.POMEGRANATE_TREE);
        var woodyGrowthPatch = features.getOrThrow(AylythConfiguredFeautres.WOODY_GROWTH_PATCH);
        var woodyGrowthBowelsPatch = features.getOrThrow(AylythConfiguredFeautres.WOODY_GROWTH_BOWELS_PATCH);
        var strewnLeavesPatch = features.getOrThrow(AylythConfiguredFeautres.STREWN_LEAVES_PATCH);
        var antlerShootsWaterPatch = features.getOrThrow(AylythConfiguredFeautres.ANTLER_SHOOTS_WATER_PATCH);
        var antlerShootsPatch = features.getOrThrow(AylythConfiguredFeautres.ANTLER_SHOOTS_PATCH);
        var woodyGrowthsWaterSelector = features.getOrThrow(AylythConfiguredFeautres.WOODY_GROWTHS_WATER_SELECTOR);
        var woodyGrowthsWaterPatch = features.getOrThrow(AylythConfiguredFeautres.WOODY_GROWTH_WATER_PATCH);
